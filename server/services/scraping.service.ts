...
const fetchShelves = async () => {
    try {
        ...
        $('div.sg-col-4-of-12.s-result-item.s-asin.sg-col-4-of-16.sg-col.sg-col-4-of-20').each((_idx: number, el: cheerio.Element) => {
            ...
            const name = shelf.find('span.a-size-base-plus.a-color-base.a-text-normal').text();
            const image = shelf.find('img.s-image').attr('src');
            const link = shelf.find('a.a-link-normal.a-text-normal').attr('href');
            const reviews = shelf.find('div.a-section.a-spacing-none.a-spacing-top-micro > div.a-row.a-size-small').children('span').last().attr('aria-label') || 'No reviews';
            const stars = shelf.find('div.a-section.a-spacing-none.a-spacing-top-micro > div > span').attr('aria-label') || 'No rating';
